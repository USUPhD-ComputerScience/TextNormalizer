...
/**
 * A WordNetManager maintains a set of WordNet indices and a set of readers for
 * accessing WordNet data files, both organized by part of speech (like the
 * WordNet files). The indices are loaded in their entirety the first time they
 * are needed (that is, the first time a lookup is done for that part of
 * speech). The data files are opened the first time they are needed, but are
 * never loaded in the their entirety.
 */
public class WordNetManager {
	 ...
	 */
	public static final String WNHOME_DEFAULT = (File.separatorChar == '\\'
	 ...
	 * @see http://wordnet.princeton.edu/wordnet/man/wnintro.1WN.html
	 */
	public static final String WNSEARCHDIR_DEFAULT = WNHOME_DEFAULT
	...
	/**
	 * All the parts of speech that have data files in WordNet. Can't just use
	 * PartOfSpeech.values() here because, for example, ADJS (type `s') doesn't
	 * have its own file.
	 */
	public static final PartOfSpeech[] ALL_POS = { PartOfSpeech.NOUN,
			PartOfSpeech.VERB, PartOfSpeech.ADJ, PartOfSpeech.ADV };

	/**
	 * Construct and return an instance of WordNet initialized with the given
	 * basepath (pathname of the ``dict'' directory). For convenience, this
	 * method will also check for a subdirectory of the given path named "dict".
	 ...
	 * environment variable (if set) with "dict" appended, otherwise the WordNet
	 * default basepath.
	 * 
	 * @see WNHOME_DEFAULT
		...
		if (!isWordNetDir(basepath)) {
			...
			if (!isWordNetDir(basepath + File.separator + "dict")) {
				throw new FileNotFoundException(
						"Invalid WordNet basepath: " + basepath);
			}
			basepath += File.separator + "dict";
		}
	...
	/**
	 * Returns true if path is a valid WordNet database directory (that is, one
	 * containing the index and data files, named "dict" by default).
	 */
	...
	/**
	 * Return the pathname of the index file for the given PartOfSpeech, for
	 * this WordNetManager.
	 */
	...
	/**
	 * Return the pathname of the data file for the given PartOfSpeech, for this
	 * WordNetManager.
	 */
	 ...
	 */
	protected Map<PartOfSpeech, Index> indexMap = new Hashtable<PartOfSpeech, Index>();
	 ...
	 * This method can be used to bring in all the WordNet indices potentially
	 * required by this WordNetManager in one go. Note however that individual
	 * lookups can still generate IOExceptions when they access the data files.
	 */
	...
	/**
	 * The mapping from parts of speech to a Reader for the corresponding data
	 * file. The file is opened and the Reader created the first time the file
	 * is needed, and left open for the lifetime of this WordNetManager.
	 */
	protected Map<PartOfSpeech, WordNetFileReader> readerMap = new Hashtable<PartOfSpeech, WordNetFileReader>();

	/**
	 * Return the WordNetFileReader for the given PartOfSpeech data file for
	 * this WordNetManager, creating (opening) it if necessary (which may throw
	 * an IOException).
	 */
	protected WordNetFileReader getDataFileReader(PartOfSpeech pos)
			throws IOException {
	...
	/**
	 * Return the IndexEntry for the given lemma and PartOfSpeech, or null if
	 * none is found.
	 * 
	 * @see prepareKey
	 */
	protected IndexEntry getIndexEntry(String lemma, PartOfSpeech pos)
			throws IOException {
	...
	/**
	 * Prepare the given string for use as a key in a WordNet index. Currently
	 * this amounts to downcasing it and replacing spaces with underscores.
	 */
			...
			if (lookupSenseNumbers) {
				for (Synset synset : synsets) {
					for (WordSense ws : synset.getWords()) {
						ws.setSenseNumber(
								lookupSenseNumber(ws.getWord(), synset));
					}
				}
			}
			if (lookupSenseKeys) {
	...
	/**
	 * Return all the Synsets for the given string (lemma) as any part of speech
	 * as a Map from the PartOfSpeech to a Synset[].
	 */
	...
	/**
	 * Lookup and return the sense number for the given Synset in the senses of
	 * the given lemma, or 0 if the Synset is not a sense of the lemma. Sense
	 * numbers come from the ordering of senses in the WordNet index file.
	 */
	public int lookupSenseNumber(String lemma, Synset synset)
			throws IOException {
	...
	/**
	 * Return the Synset pointed to by the given SynsetPointer. The handling of
	 * sense numbers and keys is taken from lookup(). Arguably it shouldn't
	 * happen here at all...
	 */
	...
	/**
	 * Finalizer for WordNetManager: Closes all data file readers.
	 */
	...
	/**
	 * Dump the contents of this WordNetManager to the given output. This is
	...
	public static void main(String[] argv) throws IOException {
		...
		*/
		...
		// PartOfSpeech pos = PartOfSpeech.forString("n");
		// Synset[] synsets = manager.lookup(word, pos);
		// for (Synset synset : synsets) {
		// printSynset(synset);
		// }
		// }
		{
			Map<PartOfSpeech, Synset[]> map = manager.lookup("address");
			for (PartOfSpeech pos : ALL_POS) {
				Synset[] synsets = map.get(pos);
				if (synsets != null) {
					for (Synset synset : synsets) {
	...
	private Set<String> addRootWordToSet(Set<String> set,
			Synset[] synsetArray) {
		if (synsetArray != null) {
			for (Synset synset : synsetArray) {
				...
				for (int i = 0; i < words.length; i++) {
					...
					String w = ws.getWord();
					if (!w.contains("_")) // don't take phrases or combined
											// words
					{
						if (w.contains("("))
							w = w.substring(0, w.length() - 4); // remove those
																// post-fix (a)
																// (p) WordNet
																// has for some
																// words
	...
	public void gatherUpSynsets(PartOfSpeech pos, Set<String> rootSet)
			throws IOException {
		int count = 0;
		Map<String, IndexEntry> map = getIndex(pos);
		for (IndexEntry ie : map.values()) {
			Synset[] synsets = ie.readSynsets(getDataFileReader(pos));
			if (lookupSenseNumbers) {
				for (Synset synset : synsets) {
					for (WordSense ws : synset.getWords()) {
			...
			addRootWordToSet(rootSet, synsets);
			count++;
			if (count % 1000 == 0)
	...
	protected static void printSynset(Synset synset) {
		...
		for (int i = 0; i < words.length; i++) {
			...
			System.out.print(w + "#" + ws.getSenseNumber() + " ("
					+ ws.getSenseKey() + ")");
		}
		System.out.println();
	 ...
	 */
	public synchronized void addWordNetManagerListener(
			WordNetManagerListener l) {
		if (listeners == null) {
	 ...
	 * listeners.
	 */
	public synchronized void removeWordNetManagerListener(
			WordNetManagerListener l) {
		listeners.remove(l);
	}
